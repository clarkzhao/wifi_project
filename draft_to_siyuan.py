...
def dnn_model(X):
    ...
    prediction=add_layer(dnn_classifi_2,wei_dnn_3,bias_dnn_3,activation_function=tf.nn.sigmoid)
...
wei_dnn_1= tf.Variable(tf.random_normal([32, 128],mean=0.0, stddev=0.05))
wei_dnn_2= tf.Variable(tf.random_normal([128, 128],mean=0.0, stddev=0.05))
wei_dnn_3=tf.Variable(tf.random_normal([128, 94],mean=0.0, stddev=0.05))
...
with tf.Session() as sess:
    ...
    for i_dnn in range(200001):
       ...
       ## connected the trained weights with a full connection network ##
        if i_dnn % 1000 == 0:
            train_loss_dnn = dnn_cross_entropy.eval({x:sample_x , y_: sample_y,weight_dnn_1_trained:w1,weight_dnn_2_trained:w2,
        ...
        if i_dnn % 1000 == 0:
            ...
            accuracy_train = sess.run(accuracy, feed_dict={x: sample_x, y_: sample_y,weight_dnn_1_trained:w1,weight_dnn_2_trained:w2,
                           weight_dnn_3_trained:w3,bias_dnn_1_trained:b1,bias_dnn_2_trained:b2,bias_dnn_3_trained:b3})
            error_train = 1 - accuracy_train

            accuracy_test = sess.run(accuracy, feed_dict={x: test_x, y_: test_y,weight_dnn_1_trained:w1,weight_dnn_2_trained:w2,
                           weight_dnn_3_trained:w3,bias_dnn_1_trained:b1,bias_dnn_2_trained:b2,bias_dnn_3_trained:b3})
