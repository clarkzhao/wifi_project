...
def eval(mall_id, timestamp):
    ...
    eval_data = data_providers.WIFIDataProvider(mall_id, 'eval', is_eval=True, batch_size=64)
...
if __name__ == '__main__':
    ...
    for mall_id in valid_mall_id:
        ...
        if i == 0:
            ...
            all = np.concatenate([all, out])
        print("The predictions for all know has shape of: ", all.shape)
        i += 1
        print("================ End evaluation for mall: {0} ================".format(mall_id))

    all = pd.DataFrame(all, columns=['row_id', 'shop_id'])
    csv_dir = os.path.join(pre_dir, 'all-eval.csv')
    all.to_csv(csv_dir, index=False)
